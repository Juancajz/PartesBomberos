...
import { 
    ...
    FaMapMarkerAlt, FaUsers, FaFire, FaFilePdf, FaCheckCircle, FaHourglassHalf, FaEdit 
} from 'react-icons/fa';
...
function HistorialPartes() {
  ...
  const currentUserId = parseInt(localStorage.getItem('usuario_id'));
  const currentUserRango = localStorage.getItem('usuario_rango') || '';
  const rangosAdmin = ['ADMIN', 'DIRECTOR', 'CAPITAN', 'COMANDANTE', 'AYUDANTE'];
        ...
        <div className="table-responsive">
            ...
            <tbody>
                {partesFiltrados.map(parte => {
                    // --- PERMISOS BLINDADOS ---
                    const respId = typeof parte.responsable_edicion === 'object' 
                                   ? parte.responsable_edicion?.id 
                                   : parte.responsable_edicion;
                    
                    const esResponsable = Number(respId) === Number(currentUserId);
                    const tieneLlaveMaestra = rangosAdmin.includes(currentUserRango.toUpperCase());
                    const puedeEditar = parte.estado === 'BORRADOR' && (esResponsable || tieneLlaveMaestra);
                    ...
                    return (
                        <tr key={parte.id}>
                            ...
                            <td className="text-center">
                                <Button variant="outline-primary" size="sm" className="rounded-circle shadow-sm me-1" onClick={() => handleShow(parte)} style={{width: 35, height: 35}} title="Ver Detalle"><FaEye /></Button>
                                ...
                                {parte.estado !== 'CERRADO' && ['DIRECTOR', 'CAPITAN', 'COMANDANTE'].includes(currentUserRango.toUpperCase()) && (
                                    <Button 
